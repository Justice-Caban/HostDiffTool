...
UPLOAD_RESPONSE_1=$(grpcurl -plaintext -d @ -proto proto/host_diff.proto -Iproto localhost:8080 hostdiff.HostService/UploadSnapshot <<EOF
{
  "filename": "$FILENAME_1",
...
if echo "$UPLOAD_RESPONSE_1" | grep -q "id"; then
  SNAPSHOT_ID_1=$(echo "$UPLOAD_RESPONSE_1" | grep -o '"id":"[0-9]*"' | cut -d':' -f2 | tr -d '"' || true)
...
UPLOAD_RESPONSE_2=$(grpcurl -plaintext -d @ -proto proto/host_diff.proto -Iproto localhost:8080 hostdiff.HostService/UploadSnapshot <<EOF
{
  "filename": "$FILENAME_2",
...
if echo "$UPLOAD_RESPONSE_2" | grep -q "id"; then
  SNAPSHOT_ID_2=$(echo "$UPLOAD_RESPONSE_2" | grep -o '"id":"[0-9]*"' | cut -d':' -f2 | tr -d '"' || true)
...
HISTORY_RESPONSE=$(grpcurl -plaintext -d @ -proto proto/host_diff.proto -import-path proto <<EOF
{
...
if echo "$HISTORY_RESPONSE" | grep -q "snapshotsList"; then
...
COMPARE_RESPONSE=$(grpcurl -plaintext -d @ -proto proto/host_diff.proto -Iproto localhost:8080 hostdiff.HostService/CompareSnapshots <<EOF
{
